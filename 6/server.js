...
const seedInitialTasks = async () => {
  try {
    ...
    if (count === 0) {
      console.log('🌱 Seeding initial MongoDB tasks collection for Practical 6...');
      await Task.create([
        {
          title: 'Full-Stack Integration Architecture',
          description: 'Wire React frontend components with central api.ts service communicating with Express backend.',
          completed: true
        },
        {
          title: 'Design Interactive Toast Notification System',
          description: 'Implement auto-dismissing toast feedback for all CRUD mutations (create, update, delete).',
          completed: true
        },
        {
          title: 'Granular Loading & Confirmation Lifecycle',
          description: 'Add per-action spinner states and deletion safety modal before executing DELETE calls.',
          completed: true
        },
        {
          title: 'Verify MongoDB Database Persistence',
          description: 'Perform full browser refresh (F5) to verify data remains intact across sessions.',
          completed: false
        }
      ]);
...
app.get('/', (req, res) => {
  res.status(200).json({
    message: '🚀 Practical 6: Full Stack Integration (React + Node + MongoDB)',
    database: {
